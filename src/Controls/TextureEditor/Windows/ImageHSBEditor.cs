...
{
    public class ImageHSBEditor : Window
    {
        private ImageEditDrawArgs Parameters;

        private ImageEditorHelper Editor;

        public ImageHSBEditor(ImageEditorHelper editorHelper, ImageEditDrawArgs args)
            : base("HSB Editor", new Vector2(400, 250))
        {
            Editor = editorHelper;
            Parameters = args;
        }
        ...
        public override void Render()
        {
            ...
            ImGuiHelper.BoldText("Brightness");

            update |= SliderBrightnessColor(ref Parameters.Brightness);

            ImGuiHelper.BoldText("Hue");

            update |= SliderHueColor(ref Parameters.Hue);

            ImGuiHelper.BoldText("Saturation");

            update |= SliderSaturationColor(ref Parameters.Saturation);

            ImGuiHelper.BoldText("Contrast");

            update |= SliderContrast(ref Parameters.Contrast);
